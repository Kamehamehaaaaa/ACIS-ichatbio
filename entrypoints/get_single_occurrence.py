...
from schema import occurrenceLookupApi
...
entrypoint= AgentEntrypoint(
    id="get_single_occurrence",
    description="Returns occurrence of a specific id from OBIS",
    parameters=None
)
...
async def run(request: str, context: ResponseContext):
    ...
    async with context.begin_process(summary="Searching Ocean Biodiversity Information System") as process:
        ...
        try:
            params = await search._generate_search_parameters(request, entrypoint, occurrenceLookupApi)
        ...
        try:
            ...
            if len(url) == 0:
                ...
                raise InstructorRetryException("Params generated but issue with generating URL")
